...
ComputeIntervals <- function(x, z, primary, suppressed, minVal = NULL, lpPackage = "lpSolve", gaussI = TRUE) {
  ...
  secondary2 <- length(published) + length(primary) + seq_len(length(secondary))
  ...
  a <- Reduce0exact(x[, published3, drop = FALSE], matrix(z[published3]), reduceByColSums = TRUE)
  ...
  else f.dir <- rep("==", nrow(f.con))
  f.rhs <- as.vector((as.matrix(a$z)))
  
  if (!is.null(minVal)) {
    f.con <- rbind(f.con, as.matrix(t(x[!a$yKnown, c(primary3, secondary3), drop = FALSE])))
    f.dir <- c(f.dir, rep(">=", length(primary3) + length(secondary3)))
    f.rhs <- c(f.rhs, rep(minVal, length(primary3) + length(secondary3)))
  }
  ...
  } else cat("Using Rsymphony for intervals...\n")
  ...
  for (j in seq_along(primary3)) {
    ...
    if (lpPackage == "lpSolve") {
      ...
      up[i] <- LpVal("max", f.obj, f.con, f.dir, f.rhs, all.int = TRUE)
    } else {
      lo[i] <- RsymphVal(max = FALSE,
                         obj = f.obj,
                         mat = f.con,
                         dir = f.dir,
                         rhs = f.rhs,
                         types = "C")
      up[i] <- RsymphVal(max = TRUE,
                         obj = f.obj,
                         mat = f.con,
                         dir = f.dir,
                         rhs = f.rhs,
  ...
  lo[c(published, primary, secondary)] <- lo[c(published2, primary2, secondary2)]
  up[c(published, primary, secondary)] <- up[c(published2, primary2, secondary2)]
